...
class fault_injector(object):
    def __init__(self, options, power_switch=None):
        ...
        if options.jtag and self.db.campaign.architecture == 'a9':
            ...
            if self.db.campaign.command:
                ...
                for i in range(self.options.iterations):
                    ...
                    if self.bbzybo:
                        ...
                        self.debugger.break_dut_after(hex(self.options.cache_sqlite.get_start_addr()), 0)
                        start_cycle = self.debugger.check_cycles()

                        print("Breaking on", self.options.cache_sqlite.get_end_addr())
                        self.debugger.break_dut_after(hex(self.options.cache_sqlite.get_end_addr()), 0)
    ...
    def inject_campaign(self, iteration_counter=None, timer=None):
        ...
        def monitor_execution(persistent_faults=False, log_time=False,
            ...
            if self.db.campaign.command:
                try:
                    ...
                    incomplete = False
                    if log_time:
                        self.db.result.execution_time = self.debugger.dut.get_timer_value()
            if self.db.campaign.output_file and self.db.result.outcome == 'In progress':
                if hasattr(self.debugger, 'aux') and self.db.campaign.aux_output_file:
        ...
        def check_latent_faults():
            for i in range(1, self.options.latent_iterations+1):
                if self.db.result.outcome == 'Latent faults' or self.db.result.outcome == 'Masked faults':
                    if self.db.campaign.aux:
                        self.db.log_event(
